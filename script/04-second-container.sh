...
echo -e "\n# Route table"
run_with_echo ip route list
echo "같은 IP 네트워크 대역에 두 개의 컨테이너가 있다. 두번째 컨테이너가 veth1 장치에 핑할때, 라우팅 테이블 첫번째가 선택되고 연결이 끊어진다."
echo "첫번째 라우트를 삭제하면 동작하겠지만, 반대로 netns0 컨테이너가 먹통이 될 것이다."
echo "또는 netns1이 다른 IP 네트워크를 선택하면 잘 작동할 것이다. 하지만, 여러 컨테이너가 하나의 IP 네트워크를 사용하는 것은 흔한 사용 케이스이다."